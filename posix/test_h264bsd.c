...
int comparePics(u8 *actualData, int width, int height, int picNum)
{
  ...
  if (!expectedData)
    expectedData = (u8 *)malloc(picSize);
  ...
  if (numErrors > 0)
    printf("%d pixels are different on frame %d.\n\n", numErrors, picNum);
  return numErrors;
}
...

  {
    ...
    // for (i = 0; cmdline->max_frames; i++)
    {
      ...
      if (i)
      {
        ...
        if (min_bytes > sizeof_coded_data - 4)
          min_bytes = sizeof_coded_data - 4;
        if (max_bytes < sizeof_coded_data - 4)
